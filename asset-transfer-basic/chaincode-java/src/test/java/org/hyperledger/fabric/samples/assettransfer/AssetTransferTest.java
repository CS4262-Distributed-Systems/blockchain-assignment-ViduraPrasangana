...
public final class AssetTransferTest {
    ...
    class DuplicateAssetTransaction {
        ...
        public void whenAssetExists() {
            ...
            Asset asset = contract.DuplicateAsset(ctx, "asset1", "asset3", "Sofia");
            assertThat(asset).isEqualTo(new Asset("asset3", "blue", 45, "Sofia", 60));

            // Cannot duplicate as already existing ID
            Throwable thrown = catchThrowable(() -> {
                contract.DuplicateAsset(ctx, "asset1", "asset1", "Sofia");
            });
            assertThat(thrown).isInstanceOf(ChaincodeException.class).hasNoCause()
                    .hasMessage("Asset asset1 already exists");
            assertThat(((ChaincodeException) thrown).getPayload()).isEqualTo("ASSET_ALREADY_EXISTS".getBytes());
        ...
        public void whenAssetDoesNotExist() {
            ...
            Throwable thrown = catchThrowable(() -> {
                contract.DuplicateAsset(ctx, "asset1", "asset3", "Sofia");
            });
